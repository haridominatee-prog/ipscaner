...
function attachWebSocketServer(server) {
  ...
  wss.on('connection', async (ws, request, query) => {
    ...
    if (type === 'agent' && key) {
      ...
      if (!agent) {
        ws.send(JSON.stringify({ type: 'error', message: 'Invalid Agent Key' }));
        return ws.close(4001, 'Invalid Agent Key');
      }
...
async function handleClientMessage(ws, userId, payload) {
  if (payload.action === 'start_scan') {
    ...
    if (!agentObj || agentObj.user_id !== userId) {
...
function broadcastToUser(userId, messageObj) {
  const clients = clientSockets.get(userId);
  if (clients) {
