...
function attachWebSocketServer(server) {
  ...
  wss.on('connection', async (ws, request, query) => {
    ...
    if (type === 'agent' && key) {
      ...
      await db.updateAgentStatus(agent.id, 'online', query.osInfo, query.localIp, query.version, query.ssid, query.signal);
...
async function handleAgentMessage(ws, agent, payload) {
  ...
  if (payload.type === 'heartbeat') {
    await db.updateAgentStatus(agent.id, 'online', payload.osInfo, payload.localIp, payload.version, payload.ssid, payload.signal);
    notifyUserAgentsChanged(agent.user_id);
    return;
  }
...
async function notifyUserAgentsChanged(userId) {
  let agents = await db.getAgentsByUserId(userId);
