...
router.post('/agent/auto-pair', async (req, res) => {
  try {
    ...
    // Clean up old offline duplicate agents with same name for user 1
    const existing = await db.getAgentsByUserId(defaultUserId);
    for (const a of existing) {
      if (a.agent_name === agentName && a.status !== 'online') {
        await db.deleteAgent(a.id, defaultUserId);
      }
    }

    const agentKey = auth.generateAgentKey();
    const agent = await db.createAgent(defaultUserId, agentName, agentKey);
...
router.get('/agents', auth.requireAuth, async (req, res) => {
  try {
    const agents = await db.getAgentsByUserId(req.user.userId);
    res.json({ agents });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/downloads/agent/:os', (req, res) => {
  ...
  if (osType === 'windows') {
    const batFile = path.join(agentDir, 'DOMScanner-Agent-Windows.bat');
    if (fs.existsSync(batFile)) {
      return res.download(batFile, 'DOMScanner-Agent-Windows.bat');
    }
  }
