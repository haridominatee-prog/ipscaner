...
let db = null;
let useMemory = false; // fallback when sqlite3 fails

// ── In-Memory Fallback Storage ───────────────────────────────────────────────
const memAgents = new Map();   // agentKey -> agent object
const memAgentsById = new Map(); // id -> agent object
let memAgentIdSeq = 1;
...
function initDB() {
  ...
  try {
    ...
    console.warn('⚠️  sqlite3 not available, using in-memory agent storage:', err.message);
    useMemory = true;
    return;
  }
...
// ── Agent Methods (with in-memory fallback) ──

async function createAgent(userId, agentName, agentKey) {
  if (useMemory) {
    const id = memAgentIdSeq++;
    const agent = { id, user_id: userId, agent_name: agentName, agent_key: agentKey, status: 'offline', local_ip: null, os_info: null, ssid: null, signal: null, version: '1.0.0', created_at: new Date().toISOString() };
    memAgents.set(agentKey, agent);
    memAgentsById.set(id, agent);
    return agent;
  }
  const res = await queryRun(
...
async function getAgentByKey(agentKey) {
  if (useMemory) return memAgents.get(agentKey) || null;
  return await queryGet('SELECT * FROM agents WHERE agent_key = ?', [agentKey]);
}

async function getAgentById(id) {
  if (useMemory) return memAgentsById.get(id) || null;
  return await queryGet('SELECT * FROM agents WHERE id = ?', [id]);
}

async function getAgentsByUserId(userId) {
  if (useMemory) return [...memAgentsById.values()].filter(a => String(a.user_id) === String(userId));
  return await queryAll('SELECT * FROM agents WHERE user_id = ? ORDER BY created_at DESC', [userId]);
}

async function updateAgentStatus(agentId, status, osInfo = null, localIp = null, version = null, ssid = null, signal = null) {
  if (useMemory) {
    const agent = memAgentsById.get(agentId);
    if (agent) {
      agent.status = status;
      if (osInfo) agent.os_info = osInfo;
      if (localIp) agent.local_ip = localIp;
      if (version) agent.version = version;
      if (ssid) agent.ssid = ssid;
      if (signal) agent.signal = signal;
  ...
  await queryRun(
    `UPDATE agents SET status = ?, last_heartbeat = ?,
     ...
     WHERE id = ?`,
    [status, now, osInfo, localIp, version, ssid, signal, agentId]
  );
}
...
async function deleteAgent(agentId, userId) {
  if (useMemory) {
    const agent = memAgentsById.get(agentId);
    if (agent && String(agent.user_id) === String(userId)) {
      memAgents.delete(agent.agent_key);
      memAgentsById.delete(agentId);
    }
    return;
  }
